...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Store one JEV profile with a hidden prompt, or pipe a JSON profile map.")
    parser.add_argument("--profile", default="profile-a", help="profile alias used only with an interactive terminal")
    parser.add_argument("--account", help="non-secret account label for this profile")
    args = parser.parse_args(argv)

    if sys.stdin.isatty():
        try:
            profile = normalize_profile(args.profile)
            secret = getpass.getpass("TypeSafe JEV API key (input hidden): ").strip()
            if not secret:
                raise ValueError("empty credential")
            account = args.account or profile_account(profile)
            write_windows_credential(profile, secret, account=account)
        except (OSError, ValueError, TypeError, CredentialStoreError):
