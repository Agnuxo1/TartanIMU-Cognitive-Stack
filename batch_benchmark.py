...
from jev_orchestrator.config import JEV_INPUT_PRICE_PER_MILLION, TYPESAFE_TIMEOUT_SECONDS
from jev_orchestrator.connection import create_jev_client
from jev_orchestrator.router import MODEL_CHOICES

client = create_jev_client(timeout=TYPESAFE_TIMEOUT_SECONDS)
state = {
...
try:
    batched = client.system_one(state=state, questions=questions, model="jev-1.13.0", timeout=TYPESAFE_TIMEOUT_SECONDS)
    separate_in = separate_out = 0
    for name, question in questions.items():
        response = client.system_one(state=state, questions={name: question}, model="jev-1.13.0", timeout=TYPESAFE_TIMEOUT_SECONDS)
