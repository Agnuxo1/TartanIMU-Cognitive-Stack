...
class FakeRouter:
    ...
    def local_initial_fallback(self, task):
        payload = _route("luna", "luna_coder", "python_debugging")
...
def test_sol_failure_can_reach_astra_only_after_sol_gate_failure():
    ...
    result = JEVOrchestrator(router=router, runtime=runtime, budget=Budget(max_luna_rounds=1)).execute("Design and implement a difficult Python architecture", uncertainty=0.8)
...
def test_agent_call_budget_blocks_sol_and_astra_after_luna():
    router = FakeRouter([_gate(0.0)], actions=("finish",), model_level="luna", agent="luna_coder")
    runtime = FakeRuntime(["Luna incomplete."])
    budget = Budget(max_luna_rounds=2, max_agent_calls=1, max_sol_calls=1, max_astra_calls=1)

    result = JEVOrchestrator(router=router, runtime=runtime, budget=budget).execute("Implement a Python change")

    assert [call[0] for call in runtime.calls] == ["luna_coder"]
    assert result.gate["answers"]["ready"]["value"] == 0.0


def test_checkpoint_budget_prevents_unreviewable_escalation():
    router = FakeRouter([_gate(0.0)], actions=("finish",), model_level="luna", agent="luna_coder")
    runtime = FakeRuntime(["Luna incomplete."])

    result = JEVOrchestrator(
        router=router,
        runtime=runtime,
        budget=Budget(max_checkpoints=1, max_luna_rounds=1),
    ).execute("Implement a Python change")

    assert [call[0] for call in runtime.calls] == ["luna_coder"]
    assert result.gate["decision_source"] == "budget_fallback"


def test_zero_token_budget_uses_local_route_without_model_call():
    router = FakeRouter([], model_level="luna", agent="luna_coder")
    runtime = FakeRuntime([])
    ...
    assert router.initial_calls == 0
    assert runtime.calls == []
    assert result.route["decision_source"] == "budget_fallback"
    assert result.gate["decision_source"] == "budget_fallback"
