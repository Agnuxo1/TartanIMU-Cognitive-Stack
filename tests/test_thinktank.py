...
class FakeRuntime:
    ...
    def run(self, agent_name, task, context="", use_web=False):
        ...
        model = {"luna_scout": "gpt-5.6-luna", "luna_reader": "gpt-5.6-luna", "sol_researcher": "gpt-5.6-sol", "sol_scientist": "gpt-5.6-sol", "astra_reviewer": "gpt-6-astra"}[agent_name]
...
def test_triggered_decision_runs_two_views_and_one_critic_with_compact_context():
    ...
    assert [call[0] for call in runtime.calls] == ["luna_scout", "luna_reader", "sol_scientist"]
    assert result.model_path == ["luna", "luna", "sol"]
...
def test_panel_is_bounded_by_agent_call_budget():
    router = FakeRouter(use_panel=True)
    runtime = FakeRuntime()
    budget = Budget(max_agent_calls=1)

    result = Thinktank(router, runtime, Telemetry("thinktank-budget"), budget).run(
        "Resolve conflicting evidence", "facts", uncertainty=0.9, conflicting_evidence=True
    )

    assert len(runtime.calls) == 1
    assert runtime.calls[0][0] == "luna_scout"
    assert result.critic is None
    assert "agent_budget_exhausted_before_next_independent_view" in result.failures
