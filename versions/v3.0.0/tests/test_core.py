...
def test_shared_jev_connector_does_not_read_project_secret_files(monkeypatch) -> None:
    ...
    monkeypatch.delenv("TYPESAFE_API_KEY_1", raising=False)
    monkeypatch.delenv("TYPESAFE_API_KEY_2", raising=False)
    monkeypatch.delenv("TYPESAFE_API_KEY_3", raising=False)
    import pytest
    from jev_orchestrator.connection import JevConnectionError
