...
from jev_orchestrator.connection import CONNECTIVITY_OBJECTIVE, _json_file, doctor, questions_from_spec, resolve_cli_input_path, resolve_credential, resolve_credentials
...
def test_relative_query_json_paths_resolve_from_agent_invocation_directory(monkeypatch, tmp_path):
    state_file = tmp_path / "state.json"
    state_file.write_text('{"task":"relative path smoke"}', encoding="utf-8")
    monkeypatch.setenv("JEV_CALLER_CWD", str(tmp_path))

    assert resolve_cli_input_path("state.json") == state_file
    assert _json_file("state.json", "state") == {"task": "relative path smoke"}


def test_absolute_query_json_paths_ignore_caller_directory(monkeypatch, tmp_path):
    absolute_path = tmp_path / "absolute.json"
    monkeypatch.setenv("JEV_CALLER_CWD", "C:/unrelated")

    assert resolve_cli_input_path(absolute_path) == absolute_path
