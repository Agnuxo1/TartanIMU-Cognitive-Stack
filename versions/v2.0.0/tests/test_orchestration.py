...
class FakeRuntime:
    ...
    def run(self, agent_name, task, context="", use_web=False):
        ...
        return AgentResult(output, agent_name, {"luna_coder": "gpt-5.6-luna", "luna_reader": "gpt-5.6-luna", "luna_scout": "gpt-5.6-luna", "sol_engineer": "gpt-5.6-sol", "sol_rights": "gpt-5.6-sol", "astra_architect": "gpt-6-astra", "astra_publisher": "gpt-6-astra"}[agent_name], 10, 5, 0, 0.001, "fake")
...
def test_jev_can_request_a_bounded_independent_luna_view_before_escalation():
    router = FakeRouter([_gate(1.0)], actions=("request_second_opinion",), model_level="luna", agent="luna_coder")
    runtime = FakeRuntime(["Primary view.", "Independent view."])
    result = JEVOrchestrator(router=router, runtime=runtime, budget=Budget(max_luna_rounds=1)).execute("Assess a non-deterministic Python design")
    assert [call[0] for call in runtime.calls] == ["luna_coder", "luna_reader"]
