...
class JEVOrchestrator:
    def __init__(self, run_name: str = "run", use_cache: bool = True, router: JevRouter | None = None, runtime: AgentRuntime | None = None, budget: Budget = DEFAULT_BUDGET) -> None:
        ...
        self.budget = budget
        self.thinktank = Thinktank(self.router, self.runtime, self.telemetry, budget)

    def deliberate(self, task: str, context: str = "", *, risk: str = "moderate", uncertainty: float = 0.0, conflicting_evidence: bool = False, primary_agent: str = "luna_scout", force: bool = False) -> ThinktankResult:
        """Run a JEV-gated single worker or bounded independent-view thinktank."""
        return self.thinktank.run(task, context, risk=risk, uncertainty=uncertainty, conflicting_evidence=conflicting_evidence, primary_agent=primary_agent, force=force)
    ...
    @staticmethod
    def _peer_luna_agent(agent: str) -> str:
        """Choose a complementary cheap role for an independent view."""
        if agent == "luna_coder":
            return "luna_reader"
        if agent == "luna_reader":
            return "luna_coder"
        return "luna_scout"

    def _run_independent_view(self, agent: str, task: str, primary_output: str, tool: str, needs_web: bool, round_number: int):
        """Delegate one bounded, independent Luna view without sharing hidden reasoning."""
        peer = self._peer_luna_agent(agent)
        context = compress_context(
            "OBJECTIVE:\n" + task +
            "\n\nPRIMARY WORKER OUTPUT TO CRITIQUE:\n" + primary_output +
            "\n\nINSTRUCTION:\nProvide an independent compact assessment. Check facts, acceptance criteria, risks, and missing evidence. Do not merely rewrite the primary output.",
            self.budget.max_handoff_chars,
        )
        self.telemetry.event("delegation_started", {"from_agent": agent, "to_agent": peer, "purpose": "independent_second_opinion", "model_level": "luna"})
        result = self._run_agent(peer, task, context, tool, needs_web, round_number, "luna_independent_view")
        self.telemetry.event("delegation_completed", {"from_agent": agent, "to_agent": peer, "purpose": "independent_second_opinion", "model_level": "luna"})
        return result
    ...
    def execute(self, task: str, context: str = "") -> OrchestrationResult:
        ...
        self.telemetry.event("deliberation_policy", {key: self._value(route, key) for key in ("jev_call_policy", "delegation_policy", "second_opinion_trigger", "thinktank_trigger", "thinktank_shape", "consensus_rule", "handoff_context", "human_checkpoint", "max_agents") if self._value(route, key) is not None})
        ...
        while rounds < self.budget.max_luna_rounds and len(model_path) <= self.budget.max_agent_calls:
            ...
            state = {"objective": task, "phase": "luna_round", "agent": agent, "model_level": "luna", "tool": tool, "findings": findings[-3:], "problems": problems, "observed_failures": observed_failures, "round": rounds, "tokens_used": self.telemetry.summary()["total_tokens"], "success_criteria": self._value(route, "success_criteria"), "deliberation_policy": {key: self._value(route, key) for key in ("delegation_policy", "second_opinion_trigger", "thinktank_trigger", "thinktank_shape", "consensus_rule", "human_checkpoint")}, "candidate_next_actions": ["continue", "change_tool", "change_agent", "request_second_opinion", "launch_thinktank", "escalate_sol", "finish"]}
            ...
            if action in {"request_second_opinion", "launch_thinktank", "thinktank"} and len(model_path) < self.budget.max_agent_calls:
                if action in {"launch_thinktank", "thinktank"}:
                    panel = self.deliberate(
                        task,
                        working_context,
                        risk="high" if any(word in task.lower() for word in ("publish", "rights", "contract", "money", "security", "submit", "irreversible")) else "moderate",
                        uncertainty=max(0.0, 1.0 - float(self._value(supervisor, "on_track") or 0.5)),
                        primary_agent=agent,
                        force=True,
                    )
                    output = panel.final_output
                    model_path.extend(panel.model_path)
                    gate = panel.gate
                    observed_failures.extend(panel.failures)
                    self.telemetry.event("thinktank_checkpoint", {"round": rounds, "model_path": panel.model_path, "gate": gate.get("answers", {}) if gate else None})
                    if gate and self._is_ready(gate, self.budget):
                        break
                    break
                primary_output = output
                independent = self._run_independent_view(agent, task, primary_output, tool, needs_web, rounds)
                model_path.append("luna")
                comparison = compress_context(
                    "PRIMARY VIEW:\n" + primary_output + "\n\nINDEPENDENT VIEW:\n" + independent.output,
                    7000,
                )
                gate = self.router.gate(task, comparison, problems)
                self.telemetry.event("consensus_gate", {"mode": action, "views": [agent, independent.agent], "rule": self._value(route, "consensus_rule") or "evidence_gate", "answers": gate.get("answers", {})})
                if self._is_ready(gate, self.budget):
                    self.telemetry.event("consensus_accepted", {"views": [agent, independent.agent], "mode": action})
                    break
                observed_failures = self._failure_evidence(state, gate, rounds, self.budget)
                self.telemetry.event("consensus_rejected", {"views": [agent, independent.agent], "mode": action, "evidence": observed_failures})
                break
