...
def credential_candidates(root: Path | None = None) -> list[Path]:
    ...
    if root is None:
        if TYPESAFE_KEY_FILE != active_root / "secrets" / "typesafe_api_key.txt":
...
def _is_auth_failure(exc: Exception) -> bool:
    ...
    return any(token in detail for token in ("401", "403", "unauthorized", "authentication", "invalid api key"))
