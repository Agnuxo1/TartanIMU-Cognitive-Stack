...
class AgentRuntime:
    ...
    def _list_model_ids(self, api_client: object | None = None) -> set[str]:
        try:
            ...
            self.telemetry.event("model_inventory_failed", {"reason": type(exc).__name__})
    ...
    def run(self, agent_name: str, task: str, context: str = "", use_web: bool = False) -> AgentResult:
        ...
        try:
            ...
            self.telemetry.event("subscription_backend_failed", {"agent": agent_name, "reason": type(subscription_error).__name__, "api_fallback_enabled": self.allow_openai_api})
            if not self.allow_openai_api:
                raise
            return self._run_api_with_fallback(agent_name, task, context, use_web)
    ...
    def _run_api_with_fallback(self, agent_name: str, task: str, context: str, use_web: bool) -> AgentResult:
        try:
            ...
            self.telemetry.event("api_unavailable", {"reason": self._backend_reason(exc)})
    ...
    def _run_codex(self, agent_name: str, task: str, context: str) -> AgentResult:
        ...
        try:
            ...
            codex_env["PYTHONIOENCODING"] = "utf-8"
        ...
        if failure or completed.returncode != 0:
            self.telemetry.event("codex_unavailable", {"reason": "codex_turn_failed" if failure else "nonzero_exit", "return_code": completed.returncode})
            raise ModelBackendUnavailable("Codex CLI failed; inspect the local CLI output for details.")
        if not output:
            output = stdout.strip()
        tier = AGENT_MODELS[agent_name]
