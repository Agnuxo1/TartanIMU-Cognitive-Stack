...
from .reflex_fabric import ReflexFabric, ReflexSignal
...
class JEVOrchestrator:
    def __init__(self, run_name: str = "run", use_cache: bool = True, router: JevRouter | None = None, runtime: AgentRuntime | None = None, budget: Budget = DEFAULT_BUDGET) -> None:
        ...
        self.budget = budget
        self.reflex = ReflexFabric(budget)
        self._checkpoints_used = 0
        self._agent_calls = 0
        self._tier_calls = {"luna": 0, "sol": 0, "astra": 0}
        self._token_baseline = 0

    def _can_call_agent(self, tier: str) -> bool:
        tier_limit = {
            "luna": self.budget.max_luna_rounds,
            "sol": self.budget.max_sol_calls,
            "astra": self.budget.max_astra_calls,
        }.get(tier, 0)
        return (
            self._agent_calls < self.budget.max_agent_calls
            and self._tier_calls.get(tier, 0) < tier_limit
            and self._tokens_used() < self.budget.max_total_tokens
        )

    def _record_agent_call(self, tier: str) -> None:
        self._agent_calls += 1
        self._tier_calls[tier] = self._tier_calls.get(tier, 0) + 1

    def _tokens_used(self) -> int:
        return max(0, int(self.telemetry.summary()["total_tokens"]) - self._token_baseline)

    def _gate(self, objective: str, result_summary: str, unresolved: list[str] | None = None) -> dict[str, Any]:
        if self._checkpoints_used >= self.budget.max_checkpoints or self._tokens_used() >= self.budget.max_total_tokens:
            reason = "checkpoint_budget_exhausted" if self._checkpoints_used >= self.budget.max_checkpoints else "token_budget_exhausted"
            self.telemetry.event(reason, {"phase": "gate", "used": self._checkpoints_used, "tokens_used": self._tokens_used()})
            return {
                "answers": {"ready": {"value": 0.0}, "needs_stronger_review": {"value": 1.0}, "quality": {"value": 0.0}},
                "model": "local-budget-fallback", "decision_source": "budget_fallback", "provenance": "local", "reason": reason,
            }
        self._checkpoints_used += 1
        return self.router.gate(objective, result_summary, unresolved)
    ...
    def _supervise(self, state: dict[str, Any]) -> dict[str, Any]:
        event = "phase_change" if state.get("phase") == "post_sol" else "luna_round"
        signal = ReflexSignal(
            event=event,
            uncertainty=float(state.get("uncertainty", 0.0) or 0.0),
            risk=str(state.get("risk", "moderate")),
            semantic_ambiguity=bool(state.get("semantic_ambiguity", False)),
            failure_observed=bool(state.get("observed_failures") or state.get("problems")),
            conflicting_evidence=bool(state.get("conflicting_evidence", False)),
            irreversible=bool(state.get("irreversible", False)),
            tokens_used=self._tokens_used(),
            checkpoints_used=self._checkpoints_used,
        )
        reflex = self.reflex.snapshot(signal)
        self.telemetry.event("reflex_decision", reflex)
        if not reflex["call_jev"]:
            checkpoint = {
                "answers": {
                    "next_action": {"value": "finish"},
                    "handoff_mode": {"value": "continue_from_luna"},
                    "on_track": {"value": 0.5},
                    "ready_for_gate": {"value": 0.0},
                },
                "decision_source": "reflex_local",
                "provenance": "local",
                "reflex_reason": reflex["reason"],
            }
            self.telemetry.event("supervision_fast_path", {"reason": reflex["reason"], "round": state.get("round")})
            return checkpoint
        if self._checkpoints_used >= self.budget.max_checkpoints:
            return {
                "answers": {"next_action": {"value": "finish"}, "on_track": {"value": 0.0}},
                "decision_source": "budget_fallback", "provenance": "local",
            }
        self._checkpoints_used += 1
    ...
    def execute(
        ...
        *,
        ...
        self._agent_calls = 0
        self._tier_calls = {"luna": 0, "sol": 0, "astra": 0}
        self._token_baseline = int(self.telemetry.summary()["total_tokens"])
        self.telemetry.event("task_started", {"objective": task, "budget": self.budget.__dict__, "risk": risk})
        ...
        if self.budget.max_checkpoints > 0 and self.budget.max_total_tokens > 0:
            self._checkpoints_used += 1
            route = self.router.initial(task, compress_context(context, 5000))
        else:
            route = self.router.local_initial_fallback(task)
        ...
        while self._can_call_agent("luna"):
            if self._tokens_used() >= self.budget.max_total_tokens:
            ...
            try:
                ...
                model_path.append("luna")
            except Exception as exc:
                problems.append(f"luna_agent_failed:{type(exc).__name__}")
                observed_failures.append("luna_agent_failed")
                self.telemetry.event("agent_failed", {"agent": agent, "phase": "luna", "reason": type(exc).__name__})
            state = {"objective": task, "phase": "luna_round", "agent": agent, "model_level": "luna", "tool": tool, "findings": findings[-3:], "problems": problems, "observed_failures": observed_failures, "round": rounds, "tokens_used": self._tokens_used(), "risk": risk, "uncertainty": uncertainty, "conflicting_evidence": conflicting_evidence, "irreversible": irreversible, "success_criteria": self._value(route, "success_criteria"), "candidate_next_actions": ["continue", "change_tool", "change_agent", "escalate_sol", "finish"]}
            ...
            if action == "finish" or rounds >= self.budget.max_luna_rounds:
                gate = self._gate(task, compress_context(output, 7000), problems)
        ...
        if not self._is_ready(gate, self.budget):
            ...
            if self._can_call_agent("sol") and self._checkpoints_used < self.budget.max_checkpoints:
                self.telemetry.event("escalation_decision", {"from": "luna", "to": "sol", "mode": mode, "evidence": observed_failures})
                self._record_agent_call("sol")
                sol_attempted = True
                try:
                    sol = self._run_agent(self._escalation_agent(task, "sol"), task, sol_context, tool, needs_web, 1, "sol")
                    output = sol.output
                    model_path.append("sol")
                    sol_state = {"objective": task, "phase": "post_sol", "agent": sol.agent, "model_level": "sol", "tool": tool, "findings": [compress_context(output, 5000)], "problems": observed_failures, "observed_failures": observed_failures, "round": 1, "tokens_used": self._tokens_used(), "risk": risk, "uncertainty": uncertainty, "conflicting_evidence": conflicting_evidence, "irreversible": irreversible, "candidate_next_actions": ["finish", "escalate_astra"]}
                    supervisor = self._supervise(sol_state)
                    gate = self._gate(task, compress_context(output, 7000), observed_failures)
                    self.telemetry.event("gate_checkpoint", {"phase": "sol", "answers": gate.get("answers", {})})
                except Exception as exc:
                    problems.append(f"sol_agent_failed:{type(exc).__name__}")
                    observed_failures.append("sol_agent_failed")
                    self.telemetry.event("agent_failed", {"agent_tier": "sol", "phase": "sol", "reason": type(exc).__name__})
                    gate = self._gate(task, compress_context(output, 7000), observed_failures)
            else:
                observed_failures.append("sol_agent_budget_exhausted")
                self.telemetry.event("budget_exhausted", {"phase": "sol", "agent_calls": self._agent_calls, "tokens_used": self._tokens_used()})

        if sol_attempted and not self._is_ready(gate, self.budget) and self._can_call_agent("astra") and self._checkpoints_used < self.budget.max_checkpoints:
            self.telemetry.event("pre_escalation", {"from": "sol", "to": "astra", "evidence": observed_failures, "gate": gate.get("answers", {})})
            self._record_agent_call("astra")
            try:
                ...
                problems.append(f"astra_agent_failed:{type(exc).__name__}")
                observed_failures.append("astra_agent_failed")
                self.telemetry.event("agent_failed", {"agent_tier": "astra", "phase": "astra", "reason": type(exc).__name__})
            gate = self._gate(task, compress_context(output, 7000), observed_failures)
